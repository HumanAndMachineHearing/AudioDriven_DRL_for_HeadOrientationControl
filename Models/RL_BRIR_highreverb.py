...
learning_rate = 0.0005  # was 0.00025
batch_size = 1024
...
class BalancedMemoryBuffer():
    ...
    def push_to_buffer(self, s, a, s_prime, r, t, azi_dev, ele_dev):
        ...
        # First index = location to assign to buffer, second index = experience number to determine where to assign
        # to buffer
...
# Randomly initiliaze weights with state_dict() and copy from policy network to target network to ensure that weights are identical
...
random_sampling_file = os.path.join(dir_random_sampling_file, "random_sampling_file_traindata_brir_highreverb.txt")
...
targnet_update_freq = 1  # number of episodes after which the target network should be updated.
...
projectname = 'Snellius_BRIR_wRMSnorm_highreverb_MemCap' + str(memory_capacity) + '_Eps' + str(
